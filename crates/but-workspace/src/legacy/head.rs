...
pub fn merge_worktree_with_workspace<'a>(
    ...
    let workspace_tree = gix_repo
        .find_commit(super::remerged_workspace_commit_v2(ctx, workspace)?)?
        .tree_id()?
        .detach();
    ...
    let workspace_tree_id = if heads.is_empty() {
        ...
        for head in &heads {
            ...
            if !merge.has_unresolved_conflicts(conflict_kind) {
                ...
                bail!("Merge conflict between projected workspace stacks");
            }
        }
...
#[instrument(level = "debug", skip(ctx, workspace))]
pub fn remerged_workspace_commit_v2(
    ctx: &Context,
    workspace: &but_graph::Workspace,
) -> Result<gix::ObjectId> {
    let repo = ctx.clone_repo_for_merging()?;
    let (workspace_tree_id, mut heads, target_commit) =
        remerged_workspace_tree_v2(&repo, workspace)?;
