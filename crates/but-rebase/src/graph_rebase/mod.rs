...
//! commits, with each reference (branch) riding as positioned data on its commit.
...
//!
//! References are POSITIONS, not entries with parent entries — see the `positions` module for the model.
//! That model is what makes the replay contract one sentence: ids are rewritten IN
//! PLACE (entry identity, parent arrays and positions never move), so no mutation
