...
fn cell(
    ...
    match entry {
        ...
        EditorIndex::Ref(_) => match graph.reference(entry) {
...
fn ref_groups(graph: &GraphEditor) -> HashMap<GroupKey, Vec<RefIndex>> {
    ...
    for entry in graph.positioned_refs() {
        ...
        out.entry((on, positions::entering(graph, entry)))
            .or_default()
...
fn find_heads(graph: &GraphEditor) -> Vec<EditorIndex> {
    ...
    graph
        ...
        .filter(|idx| match graph.positioned_on(*idx) {
            Some(on) => {
                let entering = positions::entering(graph, *idx);
                entering.is_empty()
                    && groups
                        .get(&(on, entering))
...
fn rendered_parents(graph: &GraphEditor, entry: EditorIndex) -> Vec<EditorIndex> {
    ...
    if let Some(on) = graph.positioned_on(entry) {
        let group = groups
            .get(&(on, positions::entering(graph, entry)))
            .map(Vec::as_slice)
            .unwrap_or_default();
        let below = group
            .iter()
...
fn topological_order(
    ...
    for &n in entries {
        for parent in rendered_parents(graph, n) {
    ...
    fn dfs(
        ...
        let parents: Vec<_> = rendered_parents(graph, entry)
            .into_iter()
    ...
    F: FnMut(gix::ObjectId) -> Option<String>,
{
    ...
    for entry in topological_order(graph, entries, &heads) {
        let (symbol, label) = cell(graph, entry, &mut get_title);
        let parents = rendered_parents(graph, entry)
            .into_iter()
            .filter(|p| entries.contains(p))
            .map(Ancestor::Parent)
            .collect();
...
mod tests {
    ...
    fn linear_graph() {
        ...
        add_parent_entry(&mut graph, c, d, 0);
        add_parent_entry(&mut graph, d, none, 0);
...
"#]]
    ...
    fn two_way_merge() {
        ...
        add_parent_entry(&mut graph, m, b, 1);
        // Both A and B have C as parent
        add_parent_entry(&mut graph, a, c, 0);
        add_parent_entry(&mut graph, b, c, 0);
...
"#]]
    ...
    fn three_way_merge() {
        ...
        add_parent_entry(&mut graph, m, a, 0);
        add_parent_entry(&mut graph, m, b, 1);
        add_parent_entry(&mut graph, m, c, 2);
        // All converge to D
        add_parent_entry(&mut graph, a, d, 0);
        add_parent_entry(&mut graph, b, d, 0);
        add_parent_entry(&mut graph, c, d, 0);
...
"#]]
    ...
    fn nested_merge_first_leg_forks_into_three() {
        ...
        add_parent_entry(&mut graph, m, f, 0);
        add_parent_entry(&mut graph, m, b, 1);

        // F forks into X, Y, Z
        add_parent_entry(&mut graph, f, x, 0);
        add_parent_entry(&mut graph, f, y, 1);
        add_parent_entry(&mut graph, f, z, 2);

        // X, Y, Z all converge to C
        add_parent_entry(&mut graph, x, c, 0);
        add_parent_entry(&mut graph, y, c, 0);
        add_parent_entry(&mut graph, z, c, 0);
...
"#]]
    ...
    fn four_way_merge() {
        ...
        add_parent_entry(&mut graph, m, b, 1);
        add_parent_entry(&mut graph, m, c, 2);
        add_parent_entry(&mut graph, m, d, 3);

        add_parent_entry(&mut graph, a, base, 0);
        add_parent_entry(&mut graph, b, base, 0);
        add_parent_entry(&mut graph, c, base, 0);
        add_parent_entry(&mut graph, d, base, 0);
...
"#]]
    ...
    fn asymmetric_merge_long_first_branch() {
        ...
        add_parent_entry(&mut graph, m, b, 1);
        add_parent_entry(&mut graph, a1, a2, 0);
        add_parent_entry(&mut graph, a2, a3, 0);
        add_parent_entry(&mut graph, a3, c, 0);
        add_parent_entry(&mut graph, b, c, 0);
...
"#]]
    ...
    fn consecutive_forks() {
        ...
        add_parent_entry(&mut graph, a, c, 1);

        // B forks to D, E
        add_parent_entry(&mut graph, b, d, 0);
        add_parent_entry(&mut graph, b, e, 1);

        // D, E, C all converge to F
        add_parent_entry(&mut graph, d, f, 0);
        add_parent_entry(&mut graph, e, f, 0);
        add_parent_entry(&mut graph, c, f, 0);
...
"#]]
    ...
    fn wide_merge_with_first_branch_forking_into_three() {
        ...
        add_parent_entry(&mut graph, x, d, 0);
        add_parent_entry(&mut graph, y, d, 0);
        add_parent_entry(&mut graph, z, d, 0);
        add_parent_entry(&mut graph, b, d, 0);
        add_parent_entry(&mut graph, c, d, 0);
...
"#]]
    ...
    fn fork_target_shared_with_a_sibling_branch() {
        ...
        // D forks to E and F
        add_parent_entry(&mut graph, d, e, 0);
        add_parent_entry(&mut graph, d, f, 1);

        // B -> base, E -> base, F -> base
        add_parent_entry(&mut graph, b, base, 0);
        add_parent_entry(&mut graph, e, base, 0);
        add_parent_entry(&mut graph, f, base, 0);
...
"#]]
    ...
    fn fork_with_multiple_branches_merging_to_same_point() {
        ...
        // D forks to E and G
        add_parent_entry(&mut graph, d, e, 0);
        add_parent_entry(&mut graph, d, g, 1);

        // B, C merge to G
        add_parent_entry(&mut graph, b, g, 0);
        add_parent_entry(&mut graph, c, g, 0);

        // E and G merge to F
        add_parent_entry(&mut graph, e, f, 0);
        add_parent_entry(&mut graph, g, f, 0);
...
"#]]
    ...
    fn three_way_fork_with_a_shared_target() {
        ...
        // C -> shared
        add_parent_entry(&mut graph, c, shared, 0);

        // D forks to E, F, shared (shared is also reached via C)
        add_parent_entry(&mut graph, d, e, 0);
        add_parent_entry(&mut graph, d, f, 1);
        add_parent_entry(&mut graph, d, shared, 2);

        // E, F, shared merge to base; B is left as a dangling tip.
        add_parent_entry(&mut graph, e, base, 0);
        add_parent_entry(&mut graph, f, base, 0);
        add_parent_entry(&mut graph, shared, base, 0);
