...
fn retain_real_ws_parents(
    ...
    // a parent index.
    let by_index = graph.ws_parent_kinds(commit_idx);
...
impl<'meta, M: RefMetadata> Editor<'meta, M> {
    ...
    pub fn rebase(self) -> Result<RebasedEditor<'meta, M>> {
        ...
        let (mut graph, mut history) = (graph, history);
        ...
        for commit_idx in to_pick {
            ...
            let mut ontos = match spec.preserved_parents.clone() {
                Some(ontos) => ontos,
                None => {
                    let ordered = collect_ordered_parents_with_indices(&graph, commit_idx);
            ...
            retain_real_ws_parents(&graph, &repo, commit_idx, &spec, &onto_indices, &mut ontos)?;
        ...
        let ref_edits = derive_ref_edits(&graph, &repo)?;
...
fn derive_ref_edits(graph: &EditorStore, repo: &gix::Repository) -> Result<Vec<RefEdit>> {
    ...
    for ref_idx in graph.ref_indices() {
        let record = graph
            .state_of(ref_idx.into())
            .expect("ref_indices only yields references");
        if !record.live || !record.mutable || !graph.is_positioned(ref_idx) {
    ...
    let creation_refs: Vec<gix::refs::FullName> = graph.creation_references().cloned().collect();
...
fn cherry_pick_order(graph: &EditorStore, heads: &[CommitIndex]) -> Result<VecDeque<CommitIndex>> {
    ...
    while let Some(base) = bases.pop_front() {
        for &ParentEntry { child: s, .. } in graph.children_of(base) {
    ...
    if ordered.len() != seen.len() {
        bail!("BUG: Rebase editor graph contains a cycle");
    }
    Ok(ordered)
}
...
"#]]
        ...
        fn incomplete_order_from_a_cycle_is_rejected() -> Result<()> {
            ...
            let err = cherry_pick_order(&graph, &[commit])
...
"#]]
            ...
            let ordered_from_a = cherry_pick_order(&graph, &[f, h])?;
