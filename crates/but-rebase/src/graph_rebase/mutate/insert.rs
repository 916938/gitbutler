...
impl<M: RefMetadata> Editor<'_, M> {
    ...
    fn insert_range_impl(
        ...
        if child == parent && self.graph.is_positioned(child) {
    ...
    fn place_reference(
        ...
        let place = match (side, self.graph.is_positioned(target)) {
        ...
        move_ref(&mut self.graph, ref_entry(subject)?, place);
    ...
    fn insert_range_above(
        ...
        } else if self.graph.positioned_on(target).is_some() {
    ...
    fn splice_above_reference(
        ...
        let child_commit = positions::resolve_to_commit(&self.graph, child)
            .context("Range child should resolve to a commit")?;
        let parent_commit = positions::resolve_to_commit(&self.graph, parent)
        ...
        let entry_number = if self.graph.is_positioned(parent) {
    ...
    fn interpose_into_group(
        ...
        let entering = positions::entering(&self.graph, target);
        let split = split_group(&mut self.graph, ref_entry(target)?, boundary, landing);
        ...
        redirect_entries(&mut self.graph, &entering, on_commit, landing);
        rehang_split_boundary(&mut self.graph, &split, landing);
        Ok((split, on_commit))
    }
    ...
    fn splice_above_commit(&mut self, target: EditorIndex, child: EditorIndex) -> Result<()> {
        ...
        if let Some(child_commit) = positions::resolve_to_commit(&self.graph, child) {
    ...
    fn connect_target_under(&mut self, parent: EditorIndex, target: EditorIndex) -> Result<()> {
        if self.graph.is_positioned(parent) {
            self.insert_parent(parent, target, 0)?;
        } else {
            let connect_to = match self.graph.positioned_on(target) {
    ...
    fn insert_range_below(
        ...
        let target_is_ref = self.graph.is_positioned(target);
        if target_is_ref {
        ...
        if !target_is_ref {
            ...
            let parent_number = self.graph.parent_count(target);
    ...
    fn gather_below_parents(
        ...
        if let Some(entries_to_connect) = entries_to_connect {
        ...
        } else if let Some(t_on) = self.graph.positioned_on(target) {
            ...
            let drained = self.graph.drain_parents(commit_entry(target)?);
    ...
    fn wire_parents_below(&mut self, parent: EditorIndex, gathered: BelowParents) -> Result<()> {
        ...
        if !moved_entry_ids.is_empty() {
            let restates: Vec<_> = moved_entry_ids
                .iter()
                .zip(new_orders)
                .filter_map(|(&old, new)| Some((old, self.graph.entry_id_at(parent_commit, new)?)))
                .collect();
            self.graph.restate_entries(&restates);
        }
        Ok(())
    }
    ...
    fn split_group_with_commit(
        ...
        let new_idx = self.graph.add_commit(new);
        let (split, on_commit) = self.interpose_into_group(target, new_idx, boundary)?;
        self.graph.push_parent(new_idx, on_commit);
        settle_group_lower(
            &mut self.graph,
            &split.lower,
            ParentEntry {
    ...
    fn insert_commit_impl(
        ...
        let target = self.resolve_anchor(target)?;
        let target_positioned = self.graph.is_positioned(target);
        let inner = match (side, target_positioned) {
            (InsertSide::Above, false) => {
                ...
                let target_commit = commit_entry(target)?;
                self.graph.redirect_children(target_commit, new_idx);
                self.graph.push_parent(new_idx, target_commit);
                positions::reposition_refs(
                    &mut self.graph,
                    target_commit,
                    new_idx,
            ...
            (InsertSide::Below, false) => {
                ...
                let target_commit = commit_entry(target)?;
                let new_idx = self.graph.add_commit(spec);
                self.graph.transplant_parents(target_commit, new_idx);
                self.graph.push_parent(target_commit, new_idx);
                EditorIndex::from(new_idx)
            }
    ...
    fn insert_reference_impl(
        ...
        let target = self.resolve_anchor(target)?;
        let target_positioned = self.graph.is_positioned(target);
        let new_ref = self.graph.add_reference(refname, true, false);
        match (side, target_positioned) {
            ...
            (InsertSide::Below, false) => {
                ...
                let target_commit = commit_entry(target)?;
                if let Some(parent_commit) = self.graph.parents(target_commit).first().copied() {
                    place_ref(
                        &mut self.graph,
                        new_ref,
                        RefPlace::GroupTop {
