...
pub(crate) fn place_ref(graph: &mut EditorStore, entry: RefIndex, place: RefPlace) {
    match place {
        ...
        RefPlace::Bottom(commit) => {
            let entering = positions::live_children_of(graph, commit);
            // Members that sat on the commit itself now sit on the new bottom.
            rehang_bottom(graph, commit, entry);
            graph.set_position(entry, commit, &entering, entering.len() > 1, None);
        }
        RefPlace::GroupTop {
            commit,
            entry: entering,
            ...
            let top = group_top_at(graph, commit, &entering, entry);
            graph.set_position(entry, commit, &entering, false, top);
        }
        RefPlace::Root(commit) => {
            graph.set_position(entry, commit, &[], false, None);
        }
    }
}
...
pub(crate) fn move_ref(graph: &mut EditorStore, entry: RefIndex, place: RefPlace) {
    ...
    let moving_edges = positions::entering(graph, entry);
    ...
    let (on, mut entering, below) = match place {
        RefPlace::Above(target) => {
            ...
            (t_on, positions::entering(graph, target), Some(target))
        }
        RefPlace::Below(target) => {
            let Some(t_on) = graph.positioned_on(target) else {
                return;
            ...
            graph.set_below(target, Some(entry));
            (t_on, positions::entering(graph, target), t_below)
        }
        RefPlace::Bottom(commit) => {
...
pub(crate) fn repoint_ref(graph: &mut EditorStore, entry: RefIndex, onto: CommitIndex) {
    ...
    match positions::resolve_to_commit(graph, entry) {
        Some(old_pick) if old_pick != onto => {
            ...
            redirect_entries(graph, &entering, old_pick, onto);
...
pub(crate) fn unhook_ref(graph: &mut EditorStore, entry: RefIndex, drop_edges: bool) {
    ...
    if drop_edges && let Some(commit) = positions::resolve_to_commit(graph, entry) {
        ...
        {
            if graph.parents(child).get(parent_number) == Some(&commit) {
...
pub(crate) fn transfer_stack(
    ...
    let lead_entering = positions::entering(graph, lead_ref);
...
pub(crate) fn land_stack_above(
    ...
    let bridge = positions::live_children_of(graph, bridge_pick);
    ...
    let top_below = graph
        ...
        .filter(|&entry| {
            ...
            while let Some(c) = cursor {
                ...
                cursor = graph.below_of(c);
            }
            true
        })
        .max_by_key(|&entry| positions::ref_depth(graph, entry));
    if let Some(top) = top_below {
        graph.set_below(boundary, Some(top));
    }
}
