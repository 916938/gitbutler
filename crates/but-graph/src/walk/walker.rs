...
pub(crate) fn traverse<T: RefMetadata>(
    ...
    }) = next.pop_front()
    {
        ...
        let id = info.id;
        segs.sync_seeds(&resolved_seeds, &mut seed_seg);
...
// ── The driver's stores: collected commits, and the walk's own segments ──
...
impl Segs {
    fn add(&mut self, seg: &ResolvedSeed) -> usize {
    ...
    fn sync_seeds(&mut self, resolved_seeds: &[ResolvedSeed], seed_seg: &mut Vec<usize>) {
        while seed_seg.len() < resolved_seeds.len() {
            let seg = self.add(&resolved_seeds[seed_seg.len()]);
            seed_seg.push(seg);
        }
    }
    ...
    /// The segment a queue item collects into — resolved_seeds into their seed segment, parents into the
    /// queuing commit's CURRENT segment (splits and swaps are picked up live: a split re-stamps
    /// `of`, so queued items need no rewrite).
...
fn segment_for_commit<T: RefMetadata>(
    ...
    Ok(if let Some(ix) = instr.seed {
        ...
        let seg = resolve_ref_and_meta(None, meta, Some((refs_by_id, id)), worktree_by_branch)?;
        segs.add(&seg)
    } else {
        ...
        {
            let seg = resolve_ref_and_meta(maybe_name, meta, None, worktree_by_branch)?;
            segs.add(&seg)
        } else {
            src_seg
        }
