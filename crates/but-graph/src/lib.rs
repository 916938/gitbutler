...
//! ### Reading this crate
//!
//! Start with the guided tour — one small repository through every stage, as asserted
//! snapshots: `tests/graph/tour.rs`. Then read the sources in dependency order:
//!
//! 1. **[`walk`]** — observation. The [`CommitGraph`] is the whole output; the module
//!    docs explain how the traversal decides to stop (convergence vs. pagination).
//! 2. **`build`** (private) — the gather-then-build phases, ending in the stored
//!    [`ref_layout`].
//! 3. **`projection/partition.rs`** — the engine. If you read one file, read this one.
//! 4. **`projection/derive.rs`** — the workspace assembled once, the display derived
//!    per call.
//! 5. **`but-rebase`'s `graph_rebase`** — the editor over the same arena.
//!
//! `crates/WORKSPACE_MODEL.md` holds the glossary, the model's rules, and the layering
//! guidance; `crates/but-testsupport/src/graph.rs` documents the snapshot glyphs. To
//! explore a real repository, `but-debug -C <repo> graph --stats` prints these stages
//! (`-t -t -t` times each phase).
//!
...
//!
//! [`CommitFlags::NotInRemote`] marks commits reachable from a non-remote seed — `HEAD`, the
//! workspace tip, the target's local tracking branch, or a workspace stack branch — so it is
//! absent exactly on commits owned by a remote alone. Independently, the workspace projection
//! walks each linked remote tracking branch and marks what it reaches with
...
/// Metadata-only declaration lookups, and where they must agree with the graph.
pub mod declared;
