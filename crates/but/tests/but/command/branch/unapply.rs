...
fn refuses_to_unapply_the_only_branch() {
    ...
    env.but("unapply A")
        ...
        .failure()
        .stderr_eq(str![[r#"
Failed to unapply branch. Cannot unapply the only branch in the workspace — apply another branch first, or run `but teardown` to leave the workspace altogether
