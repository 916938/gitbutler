...
3. The `Above` arm removes **all** of `C`'s incoming edges (here `A →₀ C`) and re-adds
   each onto `N`, with the *chubbiest grand-child* weight arithmetic (`max(order)+1`)
...
5. The branch moved — but **no step decided that**. `A` re-hung because it is a child of
   ...
   re-hang identically, wanted or not. That the ref follows must be *inferred* from "all
   children re-hang."
...
Stored: an arena with parent *arrays* (`W.parents = [C]`) and a ref table where `A`
*stands on* `C`; `W`'s parent entry carries the group `[A]`. Nothing reaches anything
*through* `A`.
...
2. The four-arm table `(side × is-the-target-a-reference)` decides, each arm a
   ...
   target `C`, `Above` — *"every ref sitting on it moves up"* (children rewire with
   parent numbers preserved, `reposition_refs` lifts `A` onto `N`); or target `A`,
   `Below` — *"a commit below a reference splits the group at that reference"*.
...
**Ledger:** arena + parent arrays (order *is* array position), refs-as-positions with
